...
// val hdfsURI = "hdfs://localhost:9000";
 ...
 val hdfs = org.apache.hadoop.fs.FileSystem.get(new java.net.URI(hdfsURI), hadoopConf) 
    ...
    val ssc = new StreamingContext(conf, Seconds(seconds.toInt))
    ...
    input.foreachRDD { rdd => 
      if(!rdd.isEmpty()){
          rdd.saveAsTextFile(hdfsURI+directotyName+sourcedir+(System.currentTimeMillis).toString());
        ...
        val filtered =             input.map(x => (x.split("\t")(1),x.split("\t")(2),x.split("\t")(3),x.split("\t")(4),x.split("\t")(5),x.split("\t")(6))).cache();
                     ...
                     val topNUrl = filtered.map(x => (x._1+"\t"+x._3,1)).reduceByKey(_+_)
                      ...
                      val topnurlcount = topNUrl.map(x => (x._1.split("\t")(1),x._2));
                        ...
                        //try { hdfs.delete(new org.apache.hadoop.fs.Path(directotyName+resultdir), true) } catch { case _ : Throwable => { } }
//                        try { hdfs.delete(new org.apache.hadoop.fs.Path(hdfsURI+directotyName+resultdir), true) } catch { case _ : Throwable => { } }
                          ...
                          if(!rdd.partitions.isEmpty) 
                            try { hdfs.delete(new org.apache.hadoop.fs.Path(hdfsURI+directotyName+resultdir), true) } catch { case _ : Throwable => { } }
                            rdd.saveAsTextFile(hdfsURI+directotyName+resultdir+"TopNUrl");
                          })
                     ...
                     .foreachRDD(rdd => {
                          if(!rdd.partitions.isEmpty) 
                                 rdd.saveAsTextFile(hdfsURI+directotyName+resultdir+"UserAgent");
                          })
                    ...
                    .foreachRDD(rdd => {
                          if(!rdd.partitions.isEmpty) 
                                 rdd.saveAsTextFile(hdfsURI+directotyName+resultdir+"ErrorType");
                          })
                     ...
                     .foreachRDD(rdd => {
                          if(!rdd.partitions.isEmpty) 
                                 rdd.saveAsTextFile(hdfsURI+directotyName+resultdir+"ContentType");
                          })
